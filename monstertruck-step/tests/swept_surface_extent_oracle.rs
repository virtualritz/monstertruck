...
fn boundary_samples(
    table: &Table,
    face: &monstertruck_step::load::FaceSurfaceHolder,
) -> Vec<Point3> {
...
mod matrix {
    use monstertruck_step::load::step_geometry::re_exports::{
...
fn extrusion_fixture(control_points: Vec<Point3>, degree: usize, axis: Vector3) -> Surface {
    use monstertruck_step::load::step_geometry::re_exports::{BsplineCurve, KnotVector};
...
fn extrusion_nets_are_indexed_profile_by_sweep() {
    use monstertruck_step::load::step_geometry::re_exports::TryIntoHomogeneousBsplineSurface;
